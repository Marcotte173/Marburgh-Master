    ...
    public override void Attack2(Player target)
    {
        if (AttemptToHit(target, 0))
        {
            if (target.PersonalShield)
            {
                ...
                Combat.AddCombatText(Color.MONSTER + name + Color.RESET + $" tries to " + Color.DAMAGE + "rake" + Color.RESET + " you but cannot break through your " + Color.SHIELD + "shield");
            }
            else
            {
                target.Bleed = 2;
                ...
                Combat.AddCombatText(Color.MONSTER + name + Color.RESET + $" rakes you for {Color.DAMAGE + Return.MitigatedDamage(damage, target.Mitigation) + Color.RESET} damage, causing " + Color.BLOOD + "bleeding" + Color.RESET);
            }
        }
        else Miss(target);
    }
